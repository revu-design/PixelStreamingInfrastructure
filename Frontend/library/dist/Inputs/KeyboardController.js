...
export class KeyboardController {
    ...
    constructor(toStreamerMessagesProvider, config, activeKeysProvider) {
        ...
        this.keyboardEventListenerTracker = new EventListenerTracker();
        /*
    ...
    handleOnKeyDown(keyboardEvent) {
        ...
        if (this.config.isFlagEnabled(Flags.SuppressBrowserKeys) &&
    ...
    getKeycode(keyboardEvent) {
        ...
        if (!('keyCode' in keyboardEvent)) {
            ...
            if (event.code in this.CodeToKeyCode) {
                ...
                Logger.Warning(Logger.GetStackTrace(), `Keyboard code of ${event.code} is not supported in our mapping, ignoring this key.`);
                return null;
            }
        }
        // If we made it here KeyboardEvent.keyCode is still supported so we can safely use it.
        if (keyboardEvent.keyCode === SpecialKeyCodes.shift &&
        ...
        else if (keyboardEvent.keyCode === SpecialKeyCodes.alt &&
